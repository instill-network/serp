...
function parseArgs(argv: string[]) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === '--debug-html') args.debugHtmlPath = argv[i + 1] && !argv[i + 1].startsWith('-') ? argv[++i] : 'last_serp.html';
    else if (a === '--debug-screenshot') args.debugScreenshotPath = argv[i + 1] && !argv[i + 1].startsWith('-') ? argv[++i] : 'last_serp.png';
...
Options:
  -n, --num <N>        Number of results (default 10)
      ...
      --proxy <url>    Proxy server (http:// or socks5://). Username can include modifiers like +country=us
...
async function main() {
  ...
  if (args.noStealth) process.env.SERP_DISABLE_STEALTH = 'true';

...
function renderReportHTML(data: { args: BenchArgs; results: OneRun[]; summary: any; correctness: any }) {
  ...
  const title = `serp-bench report`;
  const desc = `Benchmark report for Google SERP scraping vendors generated by serp-bench.`;
  const published = new Date().toISOString();
  const lang = (data.args.hl || 'en').split('-')[0];
  return `<!DOCTYPE html>
<html lang="${lang}">
<head>
  <meta charset="utf-8" />
  ...
  <meta name="keywords" content="serp, benchmark, google, scraping, playwright, proxy" />
  <meta name="generator" content="serp-bench" />
  ...
  ${JSON.stringify({
    ...
    '@type': 'Report',
    name: 'serp-bench report',
    description: desc,
    datePublished: published,
    inLanguage: lang,
    publisher: { '@type': 'Organization', name: 'instill.network', url: 'https://instill.network' },
    about: { '@type': 'SoftwareApplication', name: 'serp-bench' },
    keywords: ['serp','benchmark','google','scraping','playwright','proxy'],
  }, null, 2)}
  </script>
</head>
<body>
  <h1>serp-bench</h1>
  <div class="small">Generated: ${published}</div>
